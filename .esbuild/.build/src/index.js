...
var processRequest = async (payload, path) => {
  try {
    switch (path) {
      case "example-function" /* example */: {
        const result = await example_handler_default(payload);
        const status = result.ok ? HttpStatusCode2.Ok : result.error.status;
        const res = {
          statusCode: status,
          body: JSON.stringify({
            statusCode: status,
